...
> **Source d'origine et repli.** Le pipeline interroge le dépôt `JeffSackmann/tennis_atp` à chaque exécution et l'utilise dès qu'il répond. Au 20 septembre 2026, ce dépôt renvoie une erreur 404 : l'API GitHub répond `Not Found` et la liste publique des dépôts de l'auteur ne contient plus que le Match Charting Project. Le pipeline se replie donc sur une copie publique **épinglée sur le dernier commit publié par l'auteur** (`712be0c5ade693cdab9e69c23a71a0edf5a23c44`, « thru 8 jun 2026 ») ; l'empreinte d'un commit Git dépendant de tout son contenu, les fichiers sont identiques à l'original quel que soit le miroir qui les sert. Dès que le dépôt d'origine répond de nouveau, il redevient la source et les fichiers sont retéléchargés, sans intervention.
>
> Trois diagnostics sont distingués ([`pipeline/sources.py`](pipeline/sources.py)) : fichier réellement absent (404), source injoignable (réseau coupé, délai dépassé, quota, erreur serveur) et source disponible. Une erreur transitoire est réessayée, jamais interprétée comme une disparition, et si aucune source ne répond le pipeline échoue au lieu de se rabattre silencieusement sur l'instantané. Le statut constaté au dernier rafraîchissement est publié dans `overview.json` et affiché sur la page méthodologie.
...
- **`refresh-data.yml`** (chaque lundi, et à chaque modification du pipeline) : télécharge les dernières données, relance tout le pipeline, vérifie le nouveau modèle dans `onnxruntime-web` (Vitest), puis commit les JSON, le modèle et le README (`chore: met à jour les données ATP`) uniquement s'ils ont changé. Ce commit déclenche un nouveau déploiement.
